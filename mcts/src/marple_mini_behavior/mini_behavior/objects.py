...
from marple_mini_behavior.bddl import OBJECT_TO_IDX
...
class Ashcan(FurnitureObj):
    def __init__(self, width=1, height=1, color='blue', name='ashcan'):
        super(Ashcan, self).__init__('ashcan', width, height,
                                     {0, 1, 2}, color, name, can_contain={0, 1, 2})


class Bed(FurnitureObj):
    def __init__(self, width=3, height=2, color='purple', name='bed'):
        super(Bed, self).__init__('bed', width, height,
                                  {0}, color, name, can_overlap=True)


class Bin(FurnitureObj):
    def __init__(self, width=1, height=1, color='purple', name='bin'):
        super(Bin, self).__init__('bin', width, height, {
            0, 1, 2}, color, name, can_contain={0, 1, 2})


class Box(FurnitureObj):
    def __init__(self, width=2, height=2, color='tan', name='box'):
        super(Box, self).__init__('box', width, height,
                                  {0}, color, name, can_contain={0})


class Bucket(FurnitureObj):
    def __init__(self, width=1, height=2, color='l_blue', name='bucket'):
        super(Bucket, self).__init__('bucket', width,
                                     height, {0}, color, name, can_contain={0})


class Cabinet(FurnitureObj):
    def __init__(self, width=2, height=3, color='brown', name='cabinet'):
        super(Cabinet, self).__init__('cabinet', width, height, {
            0, 1, 2}, color, name, can_contain={0, 1, 2}, can_seebehind=False)


class Car(FurnitureObj):
    def __init__(self, width=3, height=2, color='blue', name='car'):
        super(Car, self).__init__('car', width, height,
                                  {0, 1}, color, name, can_contain={0})


class Chair(FurnitureObj):
    def __init__(self, width=1, height=1, color='brown', name='chair'):
        super(Chair, self).__init__(
            'chair', width, height, {0, 1}, color, name)


class Countertop(FurnitureObj):
    def __init__(self, width=3, height=2, color='tan', name='countertop'):
        super(Countertop, self).__init__('countertop', width,
                                         height, {0}, color, name, can_seebehind=True)


class Light(FurnitureObj):
    def __init__(self, color='yellow', width=1, height=1, is_open=False, name='light'):
        self.is_open = is_open
        super().__init__('light', width, height, {
            0, 1, 2}, color, name, can_overlap=False, can_seebehind=True)
        open_icon_path = os.path.join(os.path.dirname(
            __file__), f'utils/object_icons/{self.type}_open.jpg')
        self.icon_open = img_to_array(open_icon_path)

    def update(self, env):
        self.is_open = self.states['toggleable'].get_value(env)
    ...
    def render(self, img):
        if self.is_open:
            fill_coords(img, point_in_icon(
                img, self.icon_open), [255, 255, 0])
        else:
            fill_coords(img, point_in_icon(img, self.icon), [255, 255, 255])


class Door(FurnitureObj):
    def __init__(self, dir, width=1, height=1, color='yellow', is_open=False, name='door'):
        self.is_open = is_open
        self.dir = dir
...
class ElectricRefrigerator(FurnitureObj):
    def __init__(self, width=2, height=3, color='l_blue', name='electric_refrigerator'):
        super(ElectricRefrigerator, self).__init__('electric_refrigerator', width, height, {
            0, 1, 2}, color, name, can_contain={0, 1, 2}, can_seebehind=False)
...
class Wall(FurnitureObj):
    def __init__(self, color='grey'):
        super().__init__('wall', 1, 1, {0, 1, 2},
                         color, 'wall', can_seebehind=False)
...
class Shelf(FurnitureObj):
    def __init__(self, width=2, height=3, color='brown', name='shelf'):
        super(Shelf, self).__init__('shelf', width, height, {
            0, 1}, color, name, can_contain={0, 1}, can_seebehind=False)


class Shower(FurnitureObj):
    def __init__(self, width=3, height=2, color='l_blue', name='shower'):
        super(Shower, self).__init__(
            'shower', width, height, {0, 1, 2}, color, name)
...
class Toilet(FurnitureObj):
    def __init__(self, width=1, height=1, color='d_blue', name='toilet'):
        super(Toilet, self).__init__(
            'toilet', width, height, {0, 1}, color, name)

class Laundry(FurnitureObj):
    def __init__(self, width=2, height=2, color='blue', name='laundry'):
        super(Laundry, self).__init__(
            'laundry', width, height, {0, 1}, color, name)
...
class Sink(FurnitureObj):
    def __init__(self, width=2, height=2, color='blue', name='sink'):
        super(Sink, self).__init__('sink', width, height, {
            0}, color, name, can_contain={0}, can_overlap=False, can_seebehind=True)
...
class Stove(FurnitureObj):
    def __init__(self, width=3, height=2, color='grey', name='stove'):
        super(Stove, self).__init__('stove', width,
                                    height, {0, 1}, color, name, can_contain={0})
...
class Dog(FurnitureObj):
    def __init__(self, width=1, height=2, color='tan', name='dog'):
        super(Dog, self).__init__('dog', width, height, {1}, color, name)

    def render(self, img):
        fill_coords(img, point_in_icon(img, self.icon), [255, 255, 255])


class Television(FurnitureObj):
    def __init__(self, width=2, height=2, color='green', name='television'):
        super(Television, self).__init__(
            'television', width, height, {1}, color, name)
...
class Closet(FurnitureObj):
    def __init__(self, width=2, height=3, color='brown', name='closet'):
        super(Closet, self).__init__(
            'closet', width, height, {1}, color, name)
