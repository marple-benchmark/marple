...
from marple_mini_behavior.mini_behavior.utils.save import *
...
from MarpleLongModels.models.vocab import *
import json
import time
import calendar
import shutil
...
class Planner:
    ...
    def low_level_planner(self, mission_dict, agent_id, args, frames, audios, audios_symbolic, texts, step_count, time_stamp, multimodal=False, save_gif=False):
        ...
        if args.save: 
            ...
            # save init state
            print("img = self.env.render('rgb_array', tile_size=16)")
            img = self.env.render('rgb_array', tile_size=16) 
            print("img = Image.fromarray(img)")
            img = Image.fromarray(img)
            print('img.save(os.path.join(self.traj_folder, "init_state.jpeg"))')
            ...
            if type(action) == list:
                if len(action) > 1:
                    ...
                    # 3. action type: forward
                        ...
                        origin_step_size = self.env.agents[self.env.cur_agent_id].step_size
                        self.env.agents[self.env.cur_agent_id].set_step_size(action[1])
                        ...
                        self.env.agents[self.env.cur_agent_id].set_step_size(
                            origin_step_size)
