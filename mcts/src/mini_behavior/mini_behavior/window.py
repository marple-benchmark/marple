...
class Window(Window):
    ...
    def __init__(self, title):
        ...
        object_ax = self.fig.add_subplot(self.closeup[:1, :])
        object_state_ax = self.fig.add_subplot(self.closeup[1:2, :])
        furniture_ax = self.fig.add_subplot(self.closeup[2:3, :])
        furniture_state_ax = self.fig.add_subplot(self.closeup[3:4, :])

        self.closeup_axes = {'object': object_ax,
                             'object_state': object_state_ax,
                             'furniture': furniture_ax,
                             'furniture_state': furniture_state_ax,
                             }
    ...
    def set_inventory(self, env):
        ...
        def gen_inv(ax, objs):
            ...
            ax.clear()
            # ax.set_ylim(0, 1)
            ax.text(0.3, 0.3, text, rotation=0, ha='center', va='center')
    ...
    def show_closeup(self, imgs):
        for name, ax in self.closeup_axes.items():
            NAME_INT_MAP = {'furniture': 0,
                            'furniture_state': 1, 'object': 2, 'object_state': 3}
            if NAME_INT_MAP[name] >= len(imgs):
                continue
            print(NAME_INT_MAP[name], len(imgs))
