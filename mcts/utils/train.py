...
def main(args: DictConfig) -> None: 
    ...
    args.wandb.name = f"{args.model.model_name}-{timestamp}"
    args.wandb.save_dir = f"/vision/u/emilyjin/marple_long/experiments/{args.experiment.experiment_name}/{args.model.model_name}"  
    ...
    args.optimizer.lr = 1e-4 if is_low_policy_model or is_goal_low_policy_model else 1e-5
    args.optimizer.weight_decay = 0
    args.optimizer.use_lr_scheduler = True
    args.optimizer.warmup = 4 if is_low_policy_model or is_goal_low_policy_model else 20

    # set args for model
    args.model.dirpath = f"/vision/u/emilyjin/marple_long/checkpoints/data_level_1/{args.experiment.experiment_name}/{args.model.model_name}"
    ...
    args.trainer.max_epochs = int(total_steps / len(train_dataloader)) + args.optimizer.warmup if args.data.data_level == 'low' else 4
    # model 
    ...
    metrics = trainer.test(model=model, dataloaders=valid_dataloader)
    print(metrics)
    
    results_path = os.path.join("/vision/u/emilyjin/marple_long/results", f"{args.experiment.experiment_name}.json")
